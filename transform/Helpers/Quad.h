...
public:
    ...
    void init() {
        ///--- Compile the shaders
        _pid = shader.load_shaders("Quad_vshader.glsl", "Quad_fshader.glsl");
    ...
    void loadVertices(const std::vector<Vec3> &vertexArray, const std::vector<unsigned int> &indexArray) {
    ...
    void loadNormals(const std::vector<Vec3> &normalArray) {
        ///--- Vertex one vertex Array
        glBindVertexArray(_vao);
        check_error_gl();
    ...
    void loadTexCoords(const std::vector<Vec2> &tCoordArray) {
        ///--- Vertex one vertex Array
        glBindVertexArray(_vao);
        check_error_gl();
        ...
        glBufferData(GL_ARRAY_BUFFER, tCoordArray.size() * sizeof(Vec3), &tCoordArray[0], GL_STATIC_DRAW);
    ...
    void draw(Mat4x4 Model, Mat4x4 View, Mat4x4 Projection){
